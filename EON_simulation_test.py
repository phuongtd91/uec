...
class EON_Clos_Network:  
    ...
    def allocate_spectrum(self, path, slot_idx, requested_slots, connection_id, index_slot):
        """Spectrum allocation for connection"""  
        for i in range(len(path) - 1):
            # allocate from index_slot (read from traffic data) for input link
            if i == 0:
                first_link = (path[i], path[i+1])
                print(f'first_link: {first_link}\n')
                self.links[first_link][index_slot:index_slot + requested_slots] = connection_id
            else:
                # allocate from slot_idx (by first_fit_assignment) for input link
                link = (path[i], path[i+1])
    ...
    def run_simulation(self, traffic_file=None):  
        ...
        for time in range(simulation_time + 1):  
            ...
            for (index, source, destination, requested_slots, index_slot, t_req, t_hold) in traffic_data:
                ...
                if time == t_req:
                    ...
                    for path in paths:
                        ...
                        if available_slots:  
                            ...
                            self.allocate_spectrum(path, slot_idx, requested_slots, connection_id, index_slot)

                            print(f"{self.links[(path[0], path[1])]}=")
